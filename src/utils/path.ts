...
// import.meta.env.BASE_URL 末尾带斜杠,如 '/astro-blog/'
export function withBase(path: string): string {
	// 规范化 path:确保以 / 开头,去掉末尾的 /
	const normalized = path.startsWith('/') ? path : '/' + path;
	const trimmed = normalized.endsWith('/') && normalized !== '/' ? normalized.slice(0, -1) : normalized;
	const base = import.meta.env.BASE_URL; // 如 '/astro-blog/'
	const baseTrimmed = base.endsWith('/') ? base.slice(0, -1) : base; // '/astro-blog'
